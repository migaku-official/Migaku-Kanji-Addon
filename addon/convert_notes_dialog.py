...
class ConvertNotesDialog(QDialog):
    def __init__(self, old_model_id, note_ids, parent=None):
        ...
        info_lbl = QLabel(
            "Using this you can convert existing kanji cards to Migaku kanji cards.\n\n"
    ...
    def import_usr_data(self):
        ...
        keyword_fld = (
            None
            if self.keyword_box.currentIndex() < 1
            else self.keyword_box.currentText()
        )
        story_fld = (
            None if self.story_box.currentIndex() < 1 else self.story_box.currentText()
        )
    ...
    def convert(self):
            ...
            if idx > 0:
                if idx in used_indexes:
                    QMessageBox.information(
                        self,
                        "Migaku Kanji",
                        "You cannot convert a single card type to multiple kanji card types!",
                    )
                    return
        ...
        if len(used_indexes) < 1:
            QMessageBox.information(
                self, "Migaku Kanji", "Select at least one target kanji card type!"
            )
            return
        ...
        if len(used_indexes) != len(self.card_type_ords):
            r = QMessageBox.question(
                self,
                "Migaku Kanji",
                "Not all card types will be converted and will be lost. Do you want to continue?",
            )
        ...
        for ct in CardType:
            ...
            for char in char_nids:
                ...
                note.flush()
                card_id = aqt.mw.col.db.scalar(
                    "SELECT id FROM cards WHERE nid = ? AND ord = ?", nid, ord_
                )
                ...
                if move_cards:
                    aqt.mw.col.db.execute(
                        "UPDATE cards SET nid = ?, did = ? WHERE id = ?",
                        note.id,
                        deck_id,
                        card_id,
                    )
                    ...
                    aqt.mw.col.db.execute(
                        "UPDATE cards SET nid = ? WHERE id = ?", note.id, card_id
                    )
        ...
        model_id_rows = aqt.mw.col.db.all(
            "DELETE FROM cards WHERE nid in %s" % anki.utils.ids2str(self.note_ids)
        )
        model_id_rows = aqt.mw.col.db.all(
            "DELETE FROM notes WHERE id in %s" % anki.utils.ids2str(self.note_ids)
        )
        aqt.mw.col.db.commit()
    ...
    def show_modal(cls, note_ids, parent=None):
        ...
        model_id_rows = aqt.mw.col.db.all(
            "SELECT DISTINCT mid FROM notes WHERE id in %s"
            % anki.utils.ids2str(note_ids)
        )
        ...
        for ct in CardType:
            ...
            if ct_model["id"] == model_id:
                util.error_msg(
                    parent,
                    "The selected notes are already in the Migaku Kanji note type!",
                )
                return
